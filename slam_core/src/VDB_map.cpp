...
VDBMap::VDBMap(const double voxel_size,
...
std::tuple<Eigen::Vector3f, double> VDBMap::GetClosestNeighbor(const Eigen::Vector3f& query) const {
...
void VDBMap::Update(const std::vector<Eigen::Vector3f>& points, const Eigen::Isometry3d& pose) {
  ...
  std::transform(points.cbegin(), points.cend(), points_transformed.begin(),
                 [&](const auto& point) { return pose.cast<float>() * point; });
