...
class CommonState
{
  public:
    ...
    // clang-format off
    inline Eigen::Vector3d p()       const noexcept { return X_.element<0>().coeffs();   }
    inline Eigen::Matrix3d R()       const noexcept { return X_.element<1>().rotation(); }
    inline Eigen::Quaterniond quat() const noexcept { return X_.element<1>().quat();     }
    ...
    void p(const Eigen::Vector3d& in)       { X_.element<0>() = manif::R3d(in);  }
    void R(const Eigen::Matrix3d& in)       { X_.element<1>() = manif::SO3d(Eigen::Quaterniond(in)); }
    void quat(const Eigen::Quaterniond& in) { X_.element<1>() = manif::SO3d(in); }
