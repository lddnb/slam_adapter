            ...
            if (plane_ptr_->is_plane) {
                ...
                if (layer_ < max_layer_) {
                    ...
                    if (leaves_[leafnum] != nullptr) {
                        ...
                        leaves_[leafnum] =
                            new OctoTree(max_layer_, layer_ + 1, layer_point_size_, max_points_size_, max_cov_points_size_, planer_threshold_);
    ...
    ptpl& single_ptpl)
{
    ...
    if (current_octo->plane_ptr_->is_plane) {
        ...
        float dis_to_center = (plane.center(0) - p_w(0)) * (plane.center(0) - p_w(0)) + (plane.center(1) - p_w(1)) * (plane.center(1) - p_w(1)) +
...
Eigen::Matrix3d calcBodyCov(const Eigen::Vector3f& pb, const float range_inc, const float degree_inc)
{
    float range = sqrt(pb[0] * pb[0] + pb[1] * pb[1] + pb[2] * pb[2]);
    float range_var = range_inc * range_inc;
    Eigen::Matrix2d direction_var;
    direction_var << pow(sin((degree_inc * M_PI / 180.0)), 2), 0, 0, pow(sin((degree_inc * M_PI / 180.0)), 2);
...
Eigen::Matrix3d
transformLiDARCovToWorld(const Eigen::Vector3f& p_lidar, const Eigen::Isometry3d& T_i_l, const State& state, const Eigen::Matrix3d& COV_lidar)
{
    ...
    // Eq. (3)
    Eigen::Matrix3d COV_world = state.R() * COV_lidar * state.R().transpose() +
                                state.R() * (-point_crossmat) * rot_var * (-point_crossmat).transpose() * state.R().transpose() + t_var;

    return COV_world;
    // Voxel map 真实实现
    //    Eigen::Matrix3d cov_world = R_body * COV_lidar * R_body.transpose() +
