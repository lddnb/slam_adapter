...
/**
 * @brief 适配“packed xyz（stride=3）”到 nanoflann 数据集接口
 *
 * @note
 * - nanoflann 通过该接口在构建/查询时读取点坐标，因此点云内存需在 KDTree 生命周期内保持有效。
 * - 点云输入为 `std::span<const float>`，按 `x0,y0,z0,x1,y1,z1,...` 连续存放。
 */
struct PackedXyzCloudView3f {
    std::span<const float> xyz{};  ///< packed xyz（长度为 3*N）
    std::size_t count{0};          ///< 点数量（N）

    /**
     * @brief 返回点云点数量
     * @return 点数量
     */
    [[nodiscard]] inline std::size_t kdtree_get_point_count() const noexcept
    {
        return count;
    }
 ...
 *
 * @code
 * 1) 直接使用 packed xyz（x0,y0,z0,x1,y1,z1,...）
 * std::vector<float> xyz = ...; // size = 3*N
 * ms_slam::slam_core::EigenNanoFlannKdTree3f tree(std::span<const float>(xyz.data(), xyz.size()));
 * Eigen::Vector3f query = ...;
 * std::vector<std::size_t> nn_idx;
 * std::vector<float> nn_dist2;
 * tree.KnnSearch(query, 8, &nn_idx, &nn_dist2);
 *
 * 2) 配合 slam_core::PointCloud（零拷贝：positions()）
 * auto xyz = cloud.positions(); // std::span<const float>, size = 3*N
 * ms_slam::slam_core::EigenNanoFlannKdTree3f tree(xyz);
 * @endcode
 */
class EigenNanoFlannKdTree3f
{
  ...
  private:
    using Distance = nanoflann::L2_Simple_Adaptor<Scalar, PackedXyzCloudView3f>;
    using KdTree = nanoflann::KDTreeSingleIndexAdaptor<Distance, PackedXyzCloudView3f, 3, Index>;

  public:
    /**
     * @brief 构造 KDTree（可选：立即绑定点云并构建索引）
     * @param points packed xyz（长度为 3*N）
     * @param leafMaxSize 叶子节点最大点数（越大构建更快、查询略慢；常用 8~32）
     * @param nThreadBuild 构建线程数（传 0 使用 nanoflann 默认策略）
     * @return 无
     */
    explicit EigenNanoFlannKdTree3f(std::span<const Scalar> points = {}, std::size_t leafMaxSize = 10, unsigned int nThreadBuild = 0)
      ...
      nThreadBuild_(nThreadBuild)
    {
        ...
        bool res = Build();
        if (!res && !points.empty()) {
            spdlog::error("EigenNanoFlannKdTree3f failed to build KDTree with {} floats", points.size());
        }
    }
     ...
     * @return 无
     */
    void SetPoints(std::span<const Scalar> points) noexcept
    {
        cloud_.xyz = points;
        cloud_.count = (points.size() % 3 == 0) ? (points.size() / 3) : 0;
    ...
    bool Build()
    {
        if (!cloud_.xyz.empty() && (cloud_.xyz.size() % 3 != 0)) {
            spdlog::error("EigenNanoFlannKdTree3f expects xyz.size() % 3 == 0, got {}", cloud_.xyz.size());
            index_.reset();
            cloud_.count = 0;
            return false;
        }
        ...
        if (cloud_.xyz.data() == nullptr) {
            spdlog::error("EigenNanoFlannKdTree3f expects non-null xyz data");
            index_.reset();
            return false;
        }
     ...
     * @return 点数量
     */
    [[nodiscard]] std::size_t Size() const noexcept { return cloud_.kdtree_get_point_count(); }
    ...
    {
        const std::size_t size = cloud_.kdtree_get_point_count();
        if (index >= size) {
            spdlog::error("EigenNanoFlannKdTree3f::GetPoint index out of range: index={}, size={}", index, size);
            return Point::Zero();
        }
        return Point(
            cloud_.xyz[index * 3 + 0],
            cloud_.xyz[index * 3 + 1],
            cloud_.xyz[index * 3 + 2]);
    }
  ...
  private:
    ...
    PackedXyzCloudView3f cloud_{};     ///< 点云视图适配器
